...
const createSupplierRules = async (req, res) => {
  try {
    ...
    const validateImage = (req, res, next)=>{
      if(req.file){
        const allowedExtention = ["jpg", "png"]
        const fileExtenion = req.file.mimetype.split("/")[1];
        if(allowedExtention.includes(fileExtenion)){
          return next()
        }else{
          return res.status(400).json({success: false, message: "Only PNG and JPG images are allowed."})
        }
      }
      return next();
    };
    ...
    validateImage(req, res, async()=>{
      ...
      res.status(201).json({ success: true,message: "Supplier rule created successfully", data: createSupplierRule });
    })
  } catch (error) {
    console.log(error);
